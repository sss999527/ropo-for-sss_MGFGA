...
def train_target(args):
    ...
    # 将 netC 设置为评估模式，并将其参数设置为不需要梯度计算
    netC.eval()
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0:
            with torch.no_grad():
                target_features, target_labels = get_target_data(netB, netF, dset_loaders["target"])
                tsne_visualization(target_features, target_labels, iter_num)
        ...
        if args.cls_par > 0:
            ...
            if args.kd:
                kd_loss = NKDLoss()(outputs_test, outputs_t, pred_soft.long())
...
def get_target_data(netB, netF, dataloader, top_classes=10):
    features_list = []
    labels_list = []
    ...
    # Filter out the top classes
    unique_labels, label_counts = np.unique(labels, return_counts=True)
    top_classes_indices = np.argsort(label_counts)[::-1][:top_classes]
    mask = np.isin(labels, unique_labels[top_classes_indices])

    target_features = features[mask]
    target_labels = labels[mask]

    return target_features, target_labels

# def get_target_data(netB, netF, dataloader):
#     target_features = []
#     target_labels = []
#
#     for inputs, labels,_ in dataloader:
#         inputs = inputs.cuda()
#         features = netB(netF(inputs))
#         target_features.append(features.cpu().detach().numpy())
