...
class SBatchNorm1d(nn.Module):
    """Batch Normalization implemented in vanilla Python from scratch for 1D input.
    Parameters:
        ...
        affine – a boolean value that when set to True, gives the layer learnable affine parameters.
                  Default: True
        track_running_stats – a boolean value that when set to True, tracks the running mean and variance.
                             Default: True
    Shape:
        Input: (N, C)
    ...
    """
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True):
        super(SBatchNorm1d, self).__init__()
        self.num_features = num_features
        self.eps = eps
        self.momentum = momentum
        self.affine = affine
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            return (input - mean[None, :]) / torch.sqrt(var[None, :] + self.eps) * self.weight[None, :] + self.bias[None, :]
        else:
